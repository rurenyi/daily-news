...
DEFAULT_PROMPT_PROFILE = PromptProfile(
    ...
    system_prompt=(
        ...
        "headline 是适合中文视频封面的短标题，优先使用准确自然的中文译名，尽量控制在 24 个中文字符或同等长度内；"
        ...
        "所有字段都必须是纯文本，不要使用 Markdown 标题、列表、加粗、代码块、反引号或链接语法。"
    ),
    user_instructions=(
...
ANTHROPIC_PROMPT_PROFILE = PromptProfile(
    ...
    system_prompt=(
        ...
        "headline 会直接用于视频封面，必须是纯文本短标题，尽量控制在 24 个中文字符或同等长度内。"
        "所有字段都必须输出纯文本，不要使用 Markdown 标题、项目符号、编号列表、加粗、斜体、代码块、引用块或链接语法。"
    ),
    user_instructions=(
