...
一个面向个人使用场景的自动化工具：追踪 Anthropic 新文章，生成中文翻译讲解与中文音频，封装为简单视频，并自动上传到 Bilibili。

## 当前 MVP

- 发现 `https://www.anthropic.com/news` 新文章
- 抓取正文并提取元数据
- 通过可插拔 provider 生成中文翻译讲解和播报文案
- 通过 Edge TTS 生成中文音频
- 用固定背景图 + 音频合成简单 MP4
- 通过 `biliup` CLI 自动投稿到 Bilibili
- 用 SQLite 保存状态，避免重复处理和支持失败重试
- 提供本地 Web 管理界面，查看原文链接、AI 翻译讲解、发布状态和视频链接

## 技术选择

- **Python 3.12**
- **SQLite**：状态存储与去重
- **Qwen / DashScope compatible-mode API**：默认翻译讲解模型
- **edge-tts**：中文配音
...
翻译讲解提示词已经抽成了独立的站点配置层，当前会根据 `source_name` 自动选择不同提示词。
...
- **深入技术细节**
- **不对讲解正文设置字符上限**
- 更强调原文关键内容的中文翻译、术语解释、模型能力、评测、工程机制、限制和实际影响

后续如果接入别的网站，只需要继续新增对应的 prompt profile，不需要重写内容生成器本身。
...
- 文章列表页：原文链接、翻译讲解片段、发布状态、视频链接
- 文章详情页：完整翻译讲解、讲解要点、中文讲解稿、错误信息、封面和视频入口
- AI 生成的讲解文本会额外保存到 `workspace_dir\\summary\\`，方便后续复查和二次加工
